...
go build -ldflags '-w -s' -gcflags '-l' -a -o pkg/filemap-linux-amd64
upx -9 pkg/filemap-linux-amd64
chmod 777 pkg/filemap-linux-amd64
go env -w CGO_ENABLED=$CGO_ENABLED_ORI
go env -w GOOS=$GOOS_ORI
go env -w GOARCH=$GOARCH_ORI
cd ./builders/ || exit
echo "filemap-linux-amd64 success"
