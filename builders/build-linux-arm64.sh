...
go build -ldflags '-w -s' -gcflags '-l' -a -o pkg/filemap-linux-arm64
upx -9 pkg/filemap-linux-arm64
chmod 777 pkg/filemap-linux-arm64
go env -w CGO_ENABLED=$CGO_ENABLED_ORI
go env -w GOOS=$GOOS_ORI
go env -w GOARCH=$GOARCH_ORI
cd ./builders/ || exit
echo "filemap-linux-arm64 success"
